...
**Description:** Implement OAuth2 authentication requirements in MCP servers, with token forwarding through the full A2A agent chain. When an MCP tool requires authenticated access to an external service (e.g., Google APIs, GitHub, cloud providers), the MCP server should handle OAuth2 flows (authorization code, client credentials, refresh tokens) and the agent platform should propagate OAuth2 tokens through the A2A delegation chain so that downstream agents and MCP servers can access protected resources on behalf of the user.

**Current State:** The platform supports basic Bearer token forwarding: incoming `Authorization: Bearer` headers are propagated via context through A2A calls (`X-Session-ID` and `Authorization` headers). However, there is no OAuth2 flow management — no authorization code exchange, no token refresh, no per-user token storage, and no mechanism for an MCP server to request OAuth2 scopes or trigger an authorization flow.

**Key Requirements:**
- OAuth2 token management in MCP servers: support for authorization code flow (user-facing) and client credentials flow (service-to-service)
- Token storage: secure per-user token persistence with refresh token support and automatic renewal
- Authorization flow trigger: when an MCP tool requires auth and no valid token exists, return a structured response that triggers the authorization flow (redirect URL, required scopes)
- A2A token forwarding: OAuth2 tokens obtained at the entry point (web UI, API client) must propagate through the full A2A chain (agent-a → agent-b → MCP server)
- Scope-based access control: MCP tools can declare required OAuth2 scopes; the platform ensures the token has adequate scopes before calling the tool
- Multi-provider support: handle different OAuth2 providers (Google, GitHub, Azure AD, custom) per MCP server configuration

**Potential Benefits:**
- MCP tools can securely access external APIs on behalf of the user (Google Drive, GitHub, cloud consoles)
- End-to-end token propagation through the A2A chain eliminates the need for each agent to manage its own credentials
- Automatic token refresh ensures long-running agent tasks don't fail due to expired tokens
- Approval workflow integration: destructive OAuth-protected actions still require explicit approval
- Foundation for building enterprise integrations that require authenticated API access
